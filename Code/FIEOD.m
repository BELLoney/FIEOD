...
%    end
FE_xtem=mean(-log2(  sum(RM_temp,2) ./(n-1) ));
   FE_x(t,l)=FE_xtem;
...
% end
    FE_deA_tem=mean(-log2(((sum(RM_deA,2))./n)));
t=1;
while (t<=n)
   RM_deA_temp=RM_deA;
   RM_deA_temp(t,:)=[];
...
%    end
FE_deA_xtem=mean(-log2(  sum(RM_deA_temp,2) ./(n-1) ));
...
for j=1:n
    temp2=FOD_deA_Xl(j,:);
    temp1=FOD_Xl(j,:);
    FEOF(j)=1-((sum((1-temp1).*weight(j,:))+sum((1-temp2).*weightA_de(j,:)))/(2*m));
end
end
